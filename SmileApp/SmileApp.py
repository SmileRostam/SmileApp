...
def update():
    ...
    if goalkeeper_game == True :
        if keyboard.A and gk.x > 245 and main_goalkeeper_game:
            gk.x -= player_move
            
        if keyboard.D and gk.x < 775 and main_goalkeeper_game:
            gk.x += player_move
            
        if keyboard.W and gk.y > 620 and main_goalkeeper_game:
            gk.y -= 4 

        if keyboard.S and gk.y < 643 and main_goalkeeper_game:
            gk.y += 4
        #-------------------  ball moves  ------------------
        if ball_move == True :
...
def on_mouse_down(pos):
    global about_menu_enable , games_menu_enable , goalkeeper_game, button_game_enable ,ball_move , shoots , main_goalkeeper_game , pause_goalkeepeing_menu_enable , close_pause_goalkeepeing_manu , about_game_menu_enable , buttons_pause_menu ,  game_sounds , app_sounds
...
#-----------------  app sound  ------------------------
    if app_sounds and (sounds_app_button.collidepoint(pos) or sounds_game_unmute.collidepoint(pos)):
        app_sounds = False
        sounds.select.play()
    elif app_sounds == False and (sounds_app_mute.collidepoint(pos) or sounds_game_mute.collidepoint(pos)):
        app_sounds = True

    #-----------------  shoot  ----------------------
    if goalkeeper_game == True :
        main_goalkeeper_game = True
        if ball.collidepoint(pos) and main_goalkeeper_game:
            ball_move = True
            shoots += 1
            if app_sounds:
                sounds.shoot.play()
    #---------------  game pause  -------------------
        ...
        if buttons_pause_menu == True and close_pause_goalkeepeing_manu.collidepoint(pos):
            ...
            if app_sounds:
                sounds.close.play()
        if pause_goalkeepeing_menu_enable == True and about_game_button.collidepoint(pos):
            buttons_pause_menu = False
        ...
        if about_game_menu_enable == True and close_about_game_manu.collidepoint(pos):
            ...
            if app_sounds:
                sounds.close.play()
        if pause_goalkeepeing_menu_enable == True and exit_game.collidepoint(pos):
            goalkeeper_game = False
            button_game_enable = True
            pause_goalkeepeing_menu_enable = False
            main_goalkeeper_game = False
